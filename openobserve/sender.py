...
def backup_logs(logs, logger):
    ...
    logger.info(
        'Backing up your logs to openobserve-failures-%s.txt', timestamp)
    with open('openobserve-failures-{}.txt'.format(timestamp), 'a') as f:
        f.writelines('\n'.join(logs))
...
class OpenObserveSender:
    def __init__(self,
                 ...
                 number_of_retries=4,
                 retry_timeout=2):
        self.headers = {"Content-type": "application/json", "Authorization": "Basic " + base64.b64encode(bytes(username + ":" + password, "utf-8")).decode("utf-8")}
    ...
    def _drain_queue(self):
        ...
        while not last_try:
            ...
            try:
                ...
                self.stdout_logger.debug(
                    'Unexpected exception while draining queue to OpenObserve, '
